...
int readatoms(FILE *fh, int natom, int *atype)
{
    int i, ion, at=0;
    char line[100];
    ...
    for(i=0;i<natom;++i)
    {
        if( (fscanf(fh, "%s", line))!=1)
	{
	    fprintf(stderr, "Error reading atom file\n");
	    exit(1);
	}
...
int main(int argc, char **argv)
{
    ...
    if(argc<2)
    {
        fprintf(stderr, "Syntax: %s trajectory_file\n", argv[0]);
        exit(1);
    }

    if( (rdf_fh=fopen("rdf.dat", "w"))==NULL)
    {
        fprintf(stderr, "Error opening rdf.dat\n");
        exit(2);
    }

    no=451;
    olist=(int *) calloc(no, sizeof(int));
    bzero(olist, sizeof(int)*no);
...
//    crd=malloc(natom*sizeof(float *));

    /* currently hardcoded */

    hist=(int *) calloc(nbins, sizeof(int));
    gr=(float *) calloc(nbins, sizeof(float));
    nr=(float *) calloc(nbins, sizeof(float));

    bzero(hist, sizeof(int)*nbins);
    bzero(gr, sizeof(float)*nbins);
    bzero(nr, sizeof(float)*nbins);

    no=451;
//    natoms=2257;

    r=(float *) calloc(no, sizeof(float));
    bzero(r, sizeof(float)*no);
...
//	    return 1;
        ...
        for(j=0;j<no;++j)
        {
	    r[j]=dist(timestep.coords, 0, olist[j]);
	}
    ...
    close_file_read(v);
    rho=no/(4/3*PI*12*12*12);
    for(i=0;i<nbins;++i)
    {
        rin=i*binwidth;
        rout=(i+1)*binwidth;

        nideal=4.0/3.0*PI*rho*(rout*rout*rout-rin*rin*rin);
	printf("i = %d hist_i =  %d nideal = %f n = %d\n", i, hist[i], nideal, n);

        gr[i]=((float) hist[i])/nideal/n;

        if(i==0)
            nr[i]=0.0;
        else
            nr[i]=nr[i-1] + ((float)hist[i]) / n;

        fprintf(rdf_fh, "%f %d %f %f\n", i*binwidth+0.5*binwidth, hist[i], gr[i], nr[i]);
    }
}
