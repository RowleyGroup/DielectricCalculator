// modifed from dcdplugin.c to calculate diffusion coefficients

// original dcdplugin.c:
/***************************************************************************
 *cr                                                                       
...
//cccccccc total frames left for statistical average cccccccccccccccccc 
//      parameter(nsteps=mega-cutstep)
//cccccccc number of particels of one molecule, nma1 is the number of
//cccccccc paricle of cation and nma2 is anion. For neutral molecule 
//cccccccc like NMA, only nma1 is necessary. npairs is the total number
//cccccccc of molecules or (cation anion) pairs.  
//       parameter(nma1=19,nma2=0,npairs=64)
//cccccccc total number of particles in the simulation
//      parameter(natoms=(nma1+nma2)*npairs)
//cccccccc cell dimension x,y,z cccccccccccccccccccccc 
//       parameter(xl=20.5475d0,yl=xl,zl=xl)
//ccccccc time interval between neighboring frame cccccccc
//      parameter(dt=1.0d0)             
//cccccc PI cccccccccccccccccccccc
//      parameter(pi=3.1415926535d0)
//cccccc BOLTZMAN CONSTANT ccccccccccccccc     
//       parameter(bolz=1.3806503d0)     
//cccccc electron charge CONSTANT cccccccccccc
//       parameter(eq=1.60217646d0)       
//ccccccc vacuum dielectric CONSANT ccccccccccccccc
//      parameter(eps0=8.854187817d0)    
//ccccccc infinite frequency dielectric CONSTANT ccccccccc
//       parameter(epsinf=1.96844935d0)  
//cccccc simulation temperature cccccccccccccc
//      parameter(T=373.0d0)       
//       common/coor/q(30000000)

#define BOLTZ 1.38065E0
#define EQ    1.6021764
#define EPS0  8.854187817
#define EPSINF 1.96844935
#define DEFAULT_T 303.0
...
float dist(float *coords, int a, int b)
{
    float rxab, ryab, rzab, r;

    rxab=coords[3*b];
    ryab=coords[3*b+1];
    rzab=coords[3*b+2];
    //    rxab=coords[3*a  ]-coords[3*b];
    //ryab=coords[3*a+1]-coords[3*b+1];
    //rzab=coords[3*a+2]-coords[3*b+2];
    r=sqrt(rxab*rxab+ryab*ryab+rzab*rzab);
    return(r);
}

psfgen_data* psfgen_data_create() {
  char namebuf[128];
  int *countptr;
  int id;
  psfgen_data *data;
...
struct psfgen_data *readpsf(char *filename)
{
  FILE *psf_file;
  int retval;
  char msg[2048];
//  psfgen_data *psf = *(psfgen_data **)data;
  struct psfgen_data *psf;

//  psf=(struct psfgen_data *) malloc(sizeof(struct psfgen_data));
  psf=psfgen_data_create();
...
//    newhandle_msg(interp,msg);
      retval = psf_file_extract(psf->mol, psf_file);
    fclose(psf_file);
  }
...
{
    float **crd;
    FILE *trajfh, *typefh, *at_file, *rdf_fh;
    int i, j, k;
    int natom, ion, no;
    int *atnum;    /* array of atomic numbers */
    int *olist;
    ...
    C=EQ*EQ*10000000.0/(3.0*BOLTZ*T)/EPS0;

    if(argc<2)
    {
        fprintf(stderr, "Syntax: %s psf_file trajectory_file [n]\n", argv[0]);
        exit(1);
    }
    ...
    if( (output_fh=fopen("frames.dat", "w")) == NULL)
    {

    }

    charge=(float *) calloc(natoms, sizeof(float));

    nseg = hasharray_count(psf->mol->segment_hash);
    count=0;

    /* create array of atomic charges */

    for ( iseg=0; iseg<nseg; ++iseg ) 
    {
	seg = psf->mol->segment_array[iseg];
	if (! seg) continue;
    ...
    qcca=(float **) calloc(nres, sizeof(float *));

    for(i=0;i<nres;++i)
    {
	qcca[i]=(float *) calloc(3, sizeof(float));
	bzero(qcca[i], sizeof(float)*3);
    }
    ...
    sum0=(float **) calloc(dcd->nsets+1, sizeof(float *));

    for(i=0;i<dcd->nsets+1;++i)
    {
	sum0[i]=(float *) calloc(3, sizeof(float));
	bzero(sum0[i], sizeof(float)*3);
    }

    /* sum allocation */

    sum=(float *) calloc(dcd->nsets+1, sizeof(float));
    bzero(sum, sizeof(float)*(dcd->nsets+1));

    for (i=0; i<dcd->nsets; i++) 
    {   
        int rc = read_next_timestep(v, natoms, &ts);
	total_volume+=(ts.A*ts.B*ts.C);

	for(j=0;j<nres;++j)
	{
	    /* compute the center of charge for a residue */
	    ...
	    for(k=0;k<natom_res;++k)
	    {
		qcca[j][0]+=charge[offset+k]*ts.coords[3*offset+3*k];
		qcca[j][1]+=charge[offset+k]*ts.coords[3*offset+3*k+1];
		qcca[j][2]+=charge[offset+k]*ts.coords[3*offset+3*k+2];
	    }
	...
	for(j=0;j<nres;++j)
	    for(k=0;k<nres;++k)
		sum[i]+=qcca[j][0]*qcca[k][0] + qcca[j][1]*qcca[k][1] + 
		    qcca[j][2]*qcca[k][2];
	...
	for(t0=0;t0<i;++t0)
	{
	    total_dip[0]+=sum0[t0][0];
	    total_dip[1]+=sum0[t0][1];
	    total_dip[2]+=sum0[t0][2];
	    total_dip2+=sum[t0];
	}

	total_dip2/=i;
	ave_volume=total_volume / i;

	ave_dip[0]=total_dip[0]/i;
	ave_dip[1]=total_dip[1]/i;
	ave_dip[2]=total_dip[2]/i;
	
	epsr=EPSINF+total_dip2*C/ave_volume-
	    (total_dip[0]/i)*(total_dip[0]/i)*C/ave_volume -
	    (total_dip[1]/i)*(total_dip[1]/i)*C/ave_volume -
	    (total_dip[2]/i)*(total_dip[2]/i)*C/ave_volume;

    }

    density=total_mass/ave_volume*(1E4/6.0221415);
